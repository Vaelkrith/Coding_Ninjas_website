...
//   quatFromVectors(a: vec3, b: vec3, out: quat, angleFactor: number = 1) {
...
//   #init(onInit?: (sketch: InfiniteGridMenu) => void) {
//     this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: false }) || undefined;
//     const gl = this.gl;
//     if (!gl) {
...
//   console.log('[InfiniteGridMenu] building atlas — items:', this.items.map(i => i.image));

//   const loadImage = (src: string, index: number) =>
//     new Promise<HTMLImageElement>((resolve) => {
//       const url = src.startsWith('/') ? src : '/' + src;
//       console.log(`[InfiniteGridMenu] loading[${index}]`, url);
//       const img = new Image();
//       // img.crossOrigin = 'anonymous'; // <--- comment out for debugging same-origin issues
//       img.onload = () => {
//         console.log(`[InfiniteGridMenu] loaded[${index}]`, url);
//         resolve(img);
//       };
//       img.onerror = (err) => {
//         console.error(`[InfiniteGridMenu] failed to load[${index}]`, url, err);
//         // make a simple fallback canvas image so atlas still builds
...
//         pctx.fillText('Missing', cellSize / 2, cellSize / 2 + 10);
//         const phImg = new Image();
//         phImg.src = ph.toDataURL();
//         phImg.onload = () => resolve(phImg);
//         phImg.onerror = () => resolve(phImg);
//       };
//       img.src = url;
//     });

//   Promise.all(this.items.map((it, i) => loadImage(it.image, i)))
//     .then(images => {
//       images.forEach((img, i) => {
//         const x = (i % this.atlasSize!) * cellSize;
//         const y = Math.floor(i / this.atlasSize!) * cellSize;
...
//       gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas);
//       gl.generateMipmap(gl.TEXTURE_2D);
//       console.log('[InfiniteGridMenu] atlas uploaded. images:', images.length);
//     })
//     .catch(err => {
//       console.error('[InfiniteGridMenu] atlas build error', err);
//     });
// }
...
// // Replace the existing exported function component at the bottom of your file with this:
...
// interface InfiniteMenuProps {
//   items?: MenuItem[];
//   // parent should pass this to open the modal (UserModal.tsx)
//   onSelect?: (item: MenuItem) => void;
// }

// export default function InfiniteMenu({ items = [], onSelect }: InfiniteMenuProps) {
...
//       const itemIndex = index % items.length;
//       setActiveItem(items[itemIndex]);
//       // debug: uncomment if you want console feedback when active item changes
//       // console.log('Active item set to:', items[itemIndex]?.title);
//     };
...
//     return () => {
//       window.removeEventListener('resize', handleResize);
//       // optional cleanup: if your class had a dispose method call it here
//       sketch = null;
//     };
//   }, [items]);

//   // central trigger that either calls parent onSelect or falls back to link behaviour
//   const triggerSelect = () => {
...
//             aria-label={activeItem.title ? `See more about ${activeItem.title}` : "See more"}
//             style={{ pointerEvents: "auto" }}
//           >
//             {activeItem.title}
//           </button>

//           <p className={`face-description ${isMoving ? "inactive" : "active"}`} style={{ pointerEvents: "auto" }}>
...
//             className={`see-more-btn ${isMoving ? "inactive" : "active"}`}
//             aria-label={`See more about ${activeItem.title}`}
//             style={{ pointerEvents: "auto" }}
//           >
//             See More →
//           </button>
...
type LocValue = GLint | WebGLUniformLocation | null;
type Locations = {
  aModelPosition?: GLint;
  aModelNormal?: GLint;
  aModelUvs?: GLint;
  aInstanceMatrix?: GLint;
  ...
  // allow `undefined` because properties are optional
  [k: string]: LocValue | undefined;
};
...
type Buffers = {
  ...
  [k: string]: WebGLBuffer | null | Uint16Array | number[] | undefined;
};

interface DiscInstances {
  matricesArray: Float32Array;
  matrices: Float32Array[]; // each instance has a Float32Array(16)
  buffer: WebGLBuffer;
}
...
// ================= Geometry + helpers (unchanged behavior, typed) =================
...
class InfiniteGridMenu {
  ...
  discLocations?: Locations;
  discGeo?: DiscGeometry | null;
  discBuffers?: Buffers;
  discVAO?: WebGLVertexArrayObject | null;
  icoGeo?: IcosahedronGeometry | null;
  instancePositions?: vec3[];
  DISC_INSTANCE_COUNT?: number;
  discInstances?: DiscInstances | null;
  worldMatrix?: mat4 | null;
  tex?: WebGLTexture | null;
  atlasSize?: number;
  control?: ArcballControl | null;
  ...
  #init(onInit?: (sketch: InfiniteGridMenu) => void) {
    ...
    this.discGeo = new DiscGeometry(56, 1);
    this.discBuffers = this.discGeo.data as Buffers;
    this.discVAO = makeVertexArray(
      gl,
      [
        [
          makeBuffer(
            gl,
            this.discBuffers.vertices as Float32Array,
            gl.STATIC_DRAW,
          )!,
          this.discLocations.aModelPosition ?? -1,
          3,
        ],
        [
          makeBuffer(gl, this.discBuffers.uvs as Float32Array, gl.STATIC_DRAW)!,
          this.discLocations.aModelUvs ?? -1,
          2,
        ],
      ],
  ...
  #initTexture() {
    ...
    console.log(
      "[InfiniteGridMenu] building atlas — items:",
      this.items.map((i) => i.image),
    );
    ...
    const loadImage = (src: string, index: number) =>
      new Promise<HTMLImageElement>((resolve) => {
        ...
        img.onerror = (err) => {
          console.error(
            `[InfiniteGridMenu] failed to load[${index}]`,
            url,
            err,
          );
          // make a simple fallback canvas image so atlas still builds
    ...
    Promise.all(this.items.map((it, i) => loadImage(it.image, i)))
      .then((images) => {
        ...
        gl.generateMipmap(gl.TEXTURE_2D);
        console.log(
          "[InfiniteGridMenu] atlas uploaded. images:",
          images.length,
        );
      })
      .catch((err) => {
  ...
  #initDiscInstances(count: number) {
    ...
    const bytesPerMatrix = 16 * 4;
    for (let j = 0; j < mat4AttribSlotCount; ++j) {
      const baseLoc = this.discLocations?.aInstanceMatrix ?? -1;
      const loc = baseLoc + j;
      if (baseLoc === -1) continue;
      gl.enableVertexAttribArray(loc);
  ...
  #animate(deltaTime: number) {
    ...
    const instancePositions = this.instancePositions ?? [];
    const positions = instancePositions.map((p) =>
  ...
  #render() {
    ...
    const uWorld = this.discLocations?.uWorldMatrix;
    const uView = this.discLocations?.uViewMatrix;
    const uProj = this.discLocations?.uProjectionMatrix;
    const uCamera = this.discLocations?.uCameraPosition;
    const uRot = this.discLocations?.uRotationAxisVelocity;

    if (uWorld && this.worldMatrix)
      gl.uniformMatrix4fv(uWorld, false, this.worldMatrix);
    if (uView) gl.uniformMatrix4fv(uView, false, this.camera.matrices.view);
    if (uProj)
      gl.uniformMatrix4fv(uProj, false, this.camera.matrices.projection);
    if (uCamera)
      gl.uniform3f(
        uCamera,
        this.camera.position[0],
    ...
    const uItemCount = this.discLocations?.uItemCount;
    const uAtlas = this.discLocations?.uAtlasSize;
    const uFrames = this.discLocations?.uFrames;
    const uScale = this.discLocations?.uScaleFactor;
    const uTex = this.discLocations?.uTex;

    if (uItemCount) gl.uniform1i(uItemCount, this.items.length);
    if (uAtlas && this.atlasSize) gl.uniform1i(uAtlas, this.atlasSize);
    if (uFrames) gl.uniform1f(uFrames, this.#frames);
    if (uScale) gl.uniform1f(uScale, this.scaleFactor);
    if (uTex) gl.uniform1i(uTex, 0);

    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, this.tex ?? null);

    gl.bindVertexArray(this.discVAO ?? null);
    const indicesLen =
      (this.discBuffers?.indices &&
        (this.discBuffers.indices as Uint16Array).length) ||
      0;
    const instanceCount = this.DISC_INSTANCE_COUNT ?? 0;
    if (indicesLen > 0 && instanceCount > 0) {
      gl.drawElementsInstanced(
        gl.TRIANGLES,
        indicesLen,
        gl.UNSIGNED_SHORT,
  ...
  #findNearestVertexIndex() {
    ...
    let maxD = -1;
    let nearestVertexIndex = 0;
    const instPos = this.instancePositions ?? [];
    for (let i = 0; i < instPos.length; ++i) {
      const d = vec3.dot(nt, instPos[i]);
      if (d > maxD) {
        maxD = d;
        nearestVertexIndex = i;
      }
...
// ================= React component at bottom (typed props) =================
      ...
      {activeItem && (
        <>
          <button
            ...
            aria-label={
              activeItem.title
                ? `See more about ${activeItem.title}`
                : "See more"
            }
            style={{ pointerEvents: "auto" }}
          >
